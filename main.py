...
from langchain_community.document_loaders import (
    ...
    TextLoader,
    BSHTMLLoader,
    Docx2txtLoader  # Added for Word Support
)
...
class UniversalRAG:
    ...
    def load_csv(self, path):
        ...
        for idx, row in df.iterrows():
            ...
            docs.append(Document(
                ...
                metadata={"source": fname, "row": idx + 1, "type": "csv", "norm_id": norm_id}
            ))
        ...
        if is_csv_intent(query):
            ...
            for fname, bm25 in self.bm25_indexes.items():
                ...
                for idx, score in enumerate(scores):
                    ...
                    if q_id and doc.metadata["norm_id"] == q_id: score += 1000
                    if score > 0: csv_results.append((doc, score))
            csv_results = [d for d, _ in sorted(csv_results, key=lambda x: x[1], reverse=True)[:5]]
        if self.vector_db:
    ...
    def answer(self, query):
        ...
        if not all_docs: return "No matching information found.", []
        context = "\n\n---\n\n".join(d.page_content for d in all_docs)
        prompt = ChatPromptTemplate.from_template("Use ONLY context.\nContext: {context}\nQuestion: {question}\nAnswer:")
        answer = (prompt | self.llm | StrOutputParser()).invoke({"context": context, "question": query})
        def format_source(d):
            src = d.metadata.get("source", "Unknown")
            extras = []
            if "row" in d.metadata: extras.append(f"Row {d.metadata['row']}")
            if "page" in d.metadata: extras.append(f"Page {d.metadata['page'] + 1}")
...
with st.sidebar:
    ...
    if st.session_state.history:
        st.download_button("📄 CSV", export_history_csv(st.session_state.history), "chat_history.csv", "text/csv")
        st.download_button("📝 Word", export_history_word(st.session_state.history), "chat_history.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")

for h in st.session_state.history:
    with st.chat_message("user"): st.markdown(h["question"])
    with st.chat_message("assistant"):
        st.markdown(h["answer"])
        if h["sources"]: st.caption(f"Sources: {', '.join(h['sources'])}")
...
if query := st.chat_input("Ask about your files..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyzing..."):
            ...
            st.markdown(ans)
            if src: st.caption(f"Sources: {', '.join(src)}")
    st.session_state.history.append({"question": query, "answer": ans, "sources": src})